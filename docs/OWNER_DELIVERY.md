...
| Custom-symbol import of a Gold fixture into MT5 | **NOT RUN** | `mql5/Scripts/Mql5Bot/Mql5BotImportFixture.mq5` exists and self-verifies by hash round-trip; last owner run (gate_run8) refused at `SYMBOL_VOLUME_MIN` (err 5308). This release reorders the volume writes to fix that; the fix is **IMPLEMENTED-UNVERIFIED** until you re-run the gate. |
