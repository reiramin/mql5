...
class TesterConfig:
    ...
    def render_ini(self, extra_inputs: dict[str, object] | None = None,
        ...
        ``report_path`` (STAGE 5 R2, DEFECT 3): when given, it is written as
        the ``Report=`` value verbatim — the caller passes an ABSOLUTE stem
        so MT5 writes the report to exactly one known place (MT5 appends
        ``.htm``).  A bare ``Report`` is resolved relative to the terminal
        INSTALLATION directory, which is not writable without elevation, so a
        run must never rely on that default.  When None, the deterministic
        bare stem is used (the generate-ini evidence path, no run attached).
        """
        self.validate()
        inputs = dict(self.inputs)
        if extra_inputs:
            inputs.update(extra_inputs)
        report_value = report_path if report_path else self.safe_report_name
...
class RunOutcome:
    ...
    STAGE 5 R2: ``exit_code`` is the terminal process exit code (DEFECT 4 —
    a leg that crashed and a leg that ran but wrote nowhere must look
    different); ``report_location`` is the path the report was ACTUALLY found
    at (DEFECT 3 — MT5's chosen location is observed and named, never
    assumed).  Both are None when unknown.
    """
...
def report_search_paths(run_dir: Path | str, data_folder: Path | str,
                        terminal_dir: Path | str,
                        safe_report_name: str) -> list[Path]:
    """The ordered candidate locations an MT5 tester report may land at.

    STAGE 5 R2, DEFECT 3.  The run points ``Report=`` at an absolute path in
    the run_dir, so the run_dir is the intended (first) location.  MT5's own
    resolution rules for a bare/relative report have historically produced a
    file under ``<data_folder>\\tester\\`` or the terminal INSTALLATION
    directory instead; those are kept as fallbacks so a report written
    elsewhere is still found — and, crucially, its actual location named —
    rather than silently declared missing.
    """
    stem = f"{safe_report_name}.htm"
    return [Path(run_dir) / stem,
            Path(data_folder) / "tester" / stem,
            Path(data_folder) / stem,
            Path(terminal_dir) / stem]


def find_report(run_dir: Path | str, data_folder: Path | str,
                terminal_dir: Path | str,
                safe_report_name: str) -> tuple[Path | None, list[Path]]:
    """Locate the tester report across the candidate paths (DEFECT 3).

    Returns ``(found_path_or_None, searched_paths)``.  The first existing
    candidate in resolution order wins; the full searched list is always
    returned so the caller can RECORD which location hit (an observed fact
    about MT5's behaviour) or, on a miss, name every place it looked.
    """
    searched = report_search_paths(run_dir, data_folder, terminal_dir,
                                   safe_report_name)
...
def report_not_found_message(searched: list[Path]) -> str:
    """Failure text naming EVERY path searched (DEFECT 3).

    A failing leg must explain itself: when no report exists at any candidate
    location, the error lists them all so the reader knows exactly where MT5
    was expected to write and did not.
    """
    joined = "; ".join(str(p) for p in searched)
    return ("report not found — searched: " + joined
...
def run_backtest(cfg: TesterConfig, settings: RunSettings) -> RunOutcome:
    ...
    # DEFECT 3: point Report at an absolute stem in the run_dir so there is
    # exactly one intended place the report can land (MT5 appends .htm) and no
    # bare-name resolution rule to guess at.
    report_stem_abs = run_dir / cfg.safe_report_name
    ini_path = run_dir / "tester.ini"
    ini_path.write_text(cfg.render_ini(report_path=str(report_stem_abs)),
                        encoding="utf-8")

    # Start fresh at EVERY candidate location so a previous run's stale report
    # can never be mistaken for this run's output nor picked up by the fallback
    # search below.
    dest = run_dir / f"{cfg.safe_report_name}.htm"
    for stale in report_search_paths(run_dir, data_folder, terminal_dir,
                                     cfg.safe_report_name):
        try:
            if stale.exists():
                stale.unlink()
        except OSError:
            pass  # a location we cannot clean (e.g. install dir) is searched, not required
    ...
    exit_code = proc.returncode  # DEFECT 4: surface what the terminal did
    ...
    if timed_out:
        ...
        # Report may land a moment after the process exits; poll the candidate
        # locations until one appears or the grace period lapses.
        deadline = time.monotonic() + 10.0
        found, searched = find_report(run_dir, data_folder, terminal_dir,
                                      cfg.safe_report_name)
        while found is None and time.monotonic() < deadline:
            ...
            found, searched = find_report(run_dir, data_folder, terminal_dir,
                                          cfg.safe_report_name)
        if found is None:
            # DEFECT 3: name every place MT5 was expected to write and did not.
            error = report_not_found_message(searched)
        else:
            # Record WHERE it came from — an observed fact about MT5's
            # behaviour, never a silently-accepted report of unknown origin.
            report_location = str(found)
            import shutil
            if found.resolve() != dest.resolve():
                shutil.copyfile(found, dest)
            report_raw = str(dest)
            try:
                parsed = parse_report_html(found.read_text(
